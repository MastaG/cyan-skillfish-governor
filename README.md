...
- `gpu-usage` (also accepts legacy `gpu_usage`)
  ...
  - `temp-read` (`"drm"` or `"sysfs"`, default: `"drm"`): where the GPU temperature is read from. `"drm"` uses the `AMDGPU_INFO_SENSOR_GPU_TEMP` ioctl and keeps a DRM device handle open for the lifetime of the process. `"sysfs"` reads the amdgpu hwmon `temp1_input` attribute instead, so no DRM handle is opened for temperature. Both report the same sensor: the kernel defines `AMDGPU_PP_SENSOR_EDGE_TEMP = AMDGPU_PP_SENSOR_GPU_TEMP`, so the ioctl and `temp1_input` resolve to the same value. Use `"sysfs"` to avoid holding a DRM client open; combined with `method = "kernel"` or `"process"` the governor then opens no DRM device at all while sysfs is working. It degrades rather than failing: if the attribute is missing, unreadable, not an integer, or reports an implausible temperature, the governor logs a warning and uses the DRM ioctl instead — at startup on the first failed probe, or mid-run after five consecutive failures, an isolated error being ridden out by reusing the last good reading. Note that handing over does open a DRM handle, so `"sysfs"` avoids one only for as long as the attribute keeps working.
